...
```{r}
# Step 3: Plot with CI and PI for Model 1
new_data <- data.frame(Brain_Size_Species_Mean = seq(min(d$Brain_Size_Species_Mean), max(d$Brain_Size_Species_Mean), length.ou = 100))
```
...
pred1 <- predict(lm1, newdata = data.frame(Brain_Size_Species_Mean = 800), interval = "prediction", level = 0.90)
...
pred2 <- predict(lm2, newdata = data.frame(Brain_Size_Species_Mean = log(800)), interval = "prediction", level = 0.90)
...
ci1 <- predict(m1, newdata = new_data, interval = "confidence", level = 0.90)
pi1 <- predict(m1, newdata = new_data, interval = "prediction", level = 0.90)

# Create plot
g <- ggplot(data = d, aes(x = Brain_Size_Species_Mean, y = MaxLongevity_m)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, color = "blue", se = FALSE) +
  geom_line(data = cbind(new_data, ci1), aes(y = lwr), color = "blue", linetype = "dashed") +
  geom_line(data = cbind(new_data, ci1), aes(y = upr), color = "blue", linetype = "dashed") +
  geom_line(data = cbind(new_data, pi1), aes(y = lwr), color = "red", linetype = "dashed") +
  geom_line(data = cbind(new_data, pi1), aes(y = upr), color = "red", linetype = "dashed") +
  ggtitle("Regression with Confidence and Prediction Intervals") +
  theme_minimal()

print(g)

# Step 4: Estimate longevity for a species with brain size = 800 gm
estimate_800 <- predict(m1, newdata = data.frame(Brain_Size_Species_Mean = 800), 
                        interval = "prediction", level = 0.90)
cat("Estimated longevity for 800 gm brain size:", estimate_800, "\n")

# Step 5: Comparing Models
cat("\nModel 1 R-squared:", summary(m1)$r.squared)
cat("\nModel 2 R-squared:", summary(m2)$r.squared)
```

### Results Interpretation

✅ **Point Estimate of Slope (𝛽1):**

-   **Model 1 (Original Data):** The slope is interpreted as the expected increase in longevity (in months) for every 1g increase in brain size.

-   **Model 2 (Log-Transformed Data):** The slope is interpreted as the expected percent increase in longevity for a 1% increase in brain size.

✅ **Hypothesis Test (H0: 𝛽1 = 0 vs. HA: 𝛽1 ≠ 0):**

-   A small p-value in the regression summary would indicate strong evidence against the null hypothesis, supporting that there’s a significant relationship.

✅ **90% Confidence Interval (CI):**

-   The CI reflects the plausible range of slope values. If zero is **not** included, the relationship is significant.

✅ **90% Prediction Interval (PI) for a Brain Size of 800g:**

-   The PI provides a range where individual observations are expected to fall. If the interval is wide, predictions may be less reliable.

✅ **Which Model is Better?**

-   The **log-transformed model** is typically better if the data has heteroscedasticity (uneven spread of residuals) or if relationships are multiplicative rather than additive.

-   The **R-squared** value will also help determine which model explains more variance in the response variable.
